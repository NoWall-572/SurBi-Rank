# 文件路径: experiments/exp1_train_eval_gcn.py
...
def get_ground_truth_ranking(G):
    """
    【最终定论版】计算包含递归级联失效与指挥节点连通性的真实物理排名。
    严格对齐论文 Section II.C (S->C->T 级联) 与 Eq. 9 (任务层连接 K)。
    """
    n = len(G)
    ...
    for i in range(n):
        ...
        # 3. 任务层失效 (Task Layer Failure): 必须能连接到指挥节点 K (Eq. 9)
        ...
        if len(final_nodes) == 0:
            ...
            if not alive_K:
                ...
                if len(functional_nodes_local) < len(G_res) * 0.1:  # 至少 10% 的节点存活
                    final_nc = 0
                else:
                    # 提取最终功能子图的邻接矩阵
...
def find_optimal_r(G):
    """【核心修复】基于 Kendall's tau 寻找最优 r (公式31和44)"""
    n = len(G)
    ...
    for r in np.arange(0.0, 1.01, 0.1):  # 调整搜索步长
        surbi_scores = r * bi_scores + (1 - r) * si_scores
        tau, _ = stats.kendalltau(true_drops, surbi_scores)

        num_ties = np.sum(np.isclose(np.diff(surbi_scores), 0))  # 优化平局惩罚
        tau -= 0.01 * num_ties
...
def generate_random_graph():
    n = np.random.randint(50, 150)  # 调整图的规模
...
def main():
    ...
    dataset = []
    tau_list = []
    for _ in tqdm(range(500)):  # 减少数量加快演示
